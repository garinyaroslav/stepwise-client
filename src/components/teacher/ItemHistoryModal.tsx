...
import { approveExplanatoryNoteItem, downloadExplanatoryNoteFile, rejectExplanatoryNoteItem } from '@/api/endpoints';
...
export function ItemHistoryModal() {
    ...
    const [activeTab, setActiveTab] = useState<Tab>(item.status === ItemStatus.SUBMITTED ? 'review' : 'current');
    ...
    const prevCfg = statusConfig[currentVersion.previousStatus] ?? statusConfig[ItemStatus.DRAFT];
    const newCfg = statusConfig[currentVersion.newStatus] ?? statusConfig[ItemStatus.SUBMITTED];

    const tabs = (
        [
            { id: 'current' as Tab, label: 'Текущий файл', icon: FileText, show: !!currentVersion },
            { id: 'history' as Tab, label: `История (${pastHistory.length})`, icon: History, show: pastHistory.length > 0 },
            { id: 'review' as Tab, label: 'Проверка', icon: MessageSquare, show: canReview },
        ] satisfies { id: Tab; label: string; icon: React.ComponentType<any>; show: boolean }[]
    ).filter((t) => t.show);
        ...
        <Dialog open onOpenChange={(open) => !open && handleClose()}>
            <DialogContent className="max-w-2xl max-h-[88vh] flex flex-col p-0 gap-0 overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-6">
                    {message && (
                        <div className={cn('mb-5 p-4 rounded-xl flex items-start gap-3 border',
                            message.type === 'success' ? 'bg-success/10 border-success/30' : 'bg-destructive/10 border-destructive/30'
                        )}>
                    ...
                    {(activeTab === 'current' || tabs.length === 1) && currentVersion && (
                        <div className="bg-muted/60 border border-border rounded-xl p-5">
                            <div className="flex items-center justify-between gap-4 mb-4">
                                ...
                                <Button variant="outline" size="sm" onClick={() => handleDownload(currentVersion.id)}
                            ...
                            <div className="flex items-center gap-2 mb-3">
                                ...
                                <span className={cn('text-xs px-2 py-0.5 rounded-full border',
                                    newCfg.bgColor, newCfg.borderColor, newCfg.color)}>
                                    {newCfg.label}
                                </span>
                            ...
                            <div className="relative">
                                ...
                                <div className="space-y-4">
                                    {pastHistory.map((historyItem) => {
                                        ...
                                        const prevStatusCfg = statusConfig[historyItem.previousStatus] ?? statusConfig[ItemStatus.DRAFT];
                                            ...
                                            <div key={historyItem.id} className="flex gap-4">
                                                ...
                                                <div className="flex-1 bg-muted/40 border border-border rounded-xl p-4 mb-1">
                                                    <div className="flex items-start justify-between gap-3 mb-2">
                                                        ...
                                                        <Button variant="ghost" size="icon" onClick={() => handleDownload(historyItem.id)}
                                                    ...
                                                    <div className="flex items-center gap-1.5 mt-2">
                                                        <span className={cn('text-xs', prevStatusCfg.color)}>
                                                            {prevStatusCfg.label}
                                                        </span>
                                                        <ArrowRight className="w-3 h-3 text-muted-foreground" />
                        ...
                        <div className="space-y-5">
                            {currentVersion && (
                                <div className="flex items-center gap-3 p-3 bg-muted/60 border border-border rounded-xl">
                                    ...
                                    <Button variant="outline" size="sm" onClick={() => handleDownload(currentVersion.id)}
                                        className="text-xs text-primary border-primary/20 hover:bg-primary/10 hover:text-primary flex-shrink-0">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <Button onClick={handleReject} disabled={isSubmitting}
                                    variant="destructive" className="flex-1 py-3 h-auto">
                                    {isSubmitting
                ...
                <div className="px-6 py-4 border-t border-border bg-muted/30">
                    <Button variant="outline" onClick={handleClose} className="w-full">Закрыть</Button>
                </div>
            </DialogContent>
        </Dialog>
    );
