...
const statusConfig: Record<
    ItemStatus,
    { label: string; color: string; bgColor: string; borderColor: string; icon: React.ComponentType<any> }
> = {
    DRAFT: {
        label: 'Черновик',
...
export function ItemHistoryModal({ item, project, chapterTitle, onClose, onUpdate }: Props) {
    ...
    return (
        ...
        >
            ...
            {/* Modal */}
            <div className="relative bg-card border border-border rounded-2xl w-full max-w-2xl max-h-[88vh] overflow-hidden flex flex-col shadow-2xl">
                {/* Header */}
                <div className="p-6 border-b border-border">
                    <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 min-w-0">
                            {/* Chapter title */}
                            <h2 className="text-xl font-semibold text-card-foreground truncate">{chapterTitle}</h2>
                            ...
                            <div className="flex items-center gap-3 mt-2 flex-wrap">
                                ...
                                <div
                                    className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-xs font-medium ${cfg.bgColor} ${cfg.borderColor} ${cfg.color}`}
                                >
                ...
                <div className="flex-1 overflow-y-auto p-6">
                    ...
                    {message && (
                        <div
                            className={`mb-5 p-4 rounded-xl flex items-start gap-3 border ${message.type === 'success'
                        ...
                        <div>
                            <div className="bg-muted/60 border border-border rounded-xl p-5">
                                ...
                                <div className="flex items-center justify-between gap-4 mb-4">
                                    ...
                                    <button
                                        onClick={() => handleDownload(item.id, currentVersion.id)}
                                        className="flex items-center gap-2 px-3 py-2 text-sm text-primary hover:bg-primary/10 rounded-lg transition-colors border border-primary/20"
                                    >
                                ...
                                <div className="flex items-center gap-2 mb-3">
                                    <span
                                        className={`text-xs px-2 py-0.5 rounded-full border ${statusConfig[currentVersion.previousStatus].bgColor} ${statusConfig[currentVersion.previousStatus].borderColor} ${statusConfig[currentVersion.previousStatus].color}`}
                                    >
                                        {statusConfig[currentVersion.previousStatus].label}
                                    </span>
                                    <ArrowRight className="w-3 h-3 text-muted-foreground" />
                                    <span
                                        className={`text-xs px-2 py-0.5 rounded-full border ${statusConfig[currentVersion.newStatus].bgColor} ${statusConfig[currentVersion.newStatus].borderColor} ${statusConfig[currentVersion.newStatus].color}`}
                                    >
                    ...
                    {activeTab === 'history' && pastHistory.length > 0 && (
                        <div>
                            <p className="text-sm text-muted-foreground mb-4">
                                Все предыдущие версии файла в хронологическом порядке
                            </p>
                            <div className="relative">
                                ...
                                <div className="space-y-4">
                                    {pastHistory.map((historyItem, idx) => {
                                        ...
                                        return (
                                            <div key={historyItem.id} className="flex gap-4">
                                                ...
                                                <div className="relative z-10 flex-shrink-0">
                                                    <div
                                                        className={`w-8 h-8 rounded-full border-2 flex items-center justify-center ${hCfg.bgColor} ${hCfg.borderColor}`}
                                                    >
                                                        <HIcon className={`w-3.5 h-3.5 ${hCfg.color}`} />
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex-1 bg-muted/40 border border-border rounded-xl p-4 mb-1">
                                                    <div className="flex items-start justify-between gap-3 mb-2">
                                                        <div className="flex items-center gap-2 flex-wrap">
                                                            ...
                                                            <span
                                                                className={`text-xs px-2 py-0.5 rounded-full border ${hCfg.bgColor} ${hCfg.borderColor} ${hCfg.color}`}
                                                            >
                                                                {hCfg.label}
                                                            </span>
                                                        </div>
                                                        <button
                                                            onClick={() => handleDownload(item.id, historyItem.id)}
                                                    ...
                                                    <div className="flex items-center gap-1.5 mt-2">
                                                        <span className={`text-xs ${statusConfig[historyItem.previousStatus].color}`}>
                                                            {statusConfig[historyItem.previousStatus].label}
                                                        </span>
                                                        <ArrowRight className="w-3 h-3 text-muted-foreground" />
                                                        <span className={`text-xs ${hCfg.color}`}>{hCfg.label}</span>
                                                    </div>
                        ...
                        <div className="space-y-5">
                                ...
                                <div className="flex items-center gap-3 p-3 bg-muted/60 border border-border rounded-xl">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs text-muted-foreground">
                                            Загружен {new Date(currentVersion.changedAt).toLocaleString('ru-RU')}
                                        </p>
                                    </div>
                                    <button
                                        onClick={() => handleDownload(item.id, currentVersion.id)}
                                        className="flex items-center gap-1.5 px-3 py-1.5 text-xs text-primary hover:bg-primary/10 rounded-lg transition-colors border border-primary/20 flex-shrink-0"
                                    >
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    rows={5}
                                    maxLength={400}
                                    className="w-full px-4 py-3 border border-input bg-background rounded-xl focus:outline-none focus:ring-2 focus:ring-ring resize-none text-card-foreground text-sm"
                                />
                            ...
                            <div className="flex items-center gap-3">
                                <button
                                    ...
                                    className="flex-1 px-4 py-3 bg-success text-success-foreground rounded-xl hover:opacity-90 transition-opacity disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 font-medium"
                                >
                                    {isSubmitting ? (
                                ...
                                <button
                                    ...
                                    disabled={isSubmitting}
                                    className="flex-1 px-4 py-3 bg-destructive text-destructive-foreground rounded-xl hover:opacity-90 transition-opacity disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 font-medium"
                                >
                                    {isSubmitting ? (
                ...
                <div className="px-6 py-4 border-t border-border bg-muted/30">
                    <button
                        onClick={onClose}
                        className="w-full px-4 py-2.5 border border-border text-card-foreground rounded-xl hover:bg-accent transition-colors text-sm font-medium"
                    >
                        Закрыть
                    </button>
                </div>
